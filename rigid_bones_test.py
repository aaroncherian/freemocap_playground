...
from skellymodels.skeleton_models.segments import Segment
...
def enforce_rigid_bones_from_skeleton(skeleton: Skeleton) -> np.ndarray:
    ...
    bone_lengths_and_statistcs = calculate_bone_lengths_and_statistics(
        marker_data=skeleton.trajectories, segment_connections=skeleton.segments
    )

    rigid_marker_data = enforce_rigid_bones(
        marker_data=skeleton.trajectories,
...
def test_rigid_bones(freemocap_3d_data):
    mediapipe_model = create_mediapipe_skeleton_model()
    mediapipe_model.integrate_freemocap_3d_data(freemocap_3d_data)

    rigid_marker_data = enforce_rigid_bones_from_skeleton(mediapipe_model)

    # og_data = mediapipe_model.trajectories['nose']

    # mediapipe_model.integrate_rigid_marker_data(rigid_marker_data)

    # new_data = mediapipe_model.trajectories['nose']
    
    return enforce_rigid_bones_from_skeleton(mediapipe_model)
    f = 2 
...
if __name__ == '__main__':
    ...
    path_to_freemocap_folder = Path(r"D:\2024-04-25_P01\1.0_recordings\sesh_2024-04-25_15_26_21_P01_NIH_Trial2")
    path_to_freemocap_output_data = path_to_freemocap_folder/'output_data'/'mediapipe_body_3d_xyz.npy'
    path_to_save_data = path_to_freemocap_folder/'output_data'/'mediapipe_body_3d_xyz_rigid.npy'
    
    freemocap_data = np.load(path_to_freemocap_output_data)

    rigid_marker_data = test_rigid_bones(freemocap_data)

    

    np.save(path_to_save_data, rigid_marker_data)
