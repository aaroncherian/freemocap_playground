...
def calculate_step_length(heel_data_dict: dict):
    ...
    for i in range(min(len(left_heel_strike_positions), len(right_heel_strike_positions)) - 1):
        left_step_length = abs(left_heel_strike_positions[i + 1] - right_heel_strike_positions[i])
        right_step_length = abs(right_heel_strike_positions[i + 1] - left_heel_strike_positions[i])
...
if __name__ == '__main__':
    ...
    for session_id, label in zip(session_id_list, label_list):
        ...
        step_lengths = calculate_step_length(heel_data_dict)
        f = 2
