...
if __name__ == '__main__':
    ...
    # Path(r"D:\2025-04-23_atc_testing\freemocap\2025-04-23_19-11-05-612Z_atc_test_walk_trial_2")
    aligned_session = Path(__file__).parents[0]/'data'/'aligned_walk'

    aligned_charuco_path = Path(__file__).parents[0]/'data'/'aligned_walk'/'aligned_charuco_3d.npy'
    aligned_charuco = np.load(aligned_charuco_path)

    aligned_charuco_frame = aligned_charuco[-1,:,:]
    ...
    for system, path_name in path_dict.items():
        data = np.load(path_name/'mediapipe_3dData_numFrames_numTrackedPoints_spatialXYZ.npy')[:,0:33,:]
